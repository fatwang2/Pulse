...
test("server-renders the full bilingual release timeline", async () => {
  ...
  assert.match(html, /id="v0-6-3"/);
  assert.match(html, /href="#v0-6-3"/);
  assert.match(html, /id="v0-1-0"/);
  assert.match(html, /href="#v0-1-0"/);
  assert.ok(html.indexOf("0.6.3") < html.indexOf("0.1.0"));
  ...
  assert.equal(releaseEntries.length, 17);
});
...
test("changelog shares the remembered language selection", async () => {
  ...
  assert.match(page, /localStorage\.getItem\("pulse-language"\)/);
  assert.match(page, /localStorage\.setItem\("pulse-language", nextLanguage\)/);
  assert.match(releaseData, /version: "0\.6\.3"/);
...
test("redirects the stable download URL to a versioned request", async () => {
  ...
  assert.equal(
    ...
    "http://localhost/download?version=0.6.3",
  );
});
...
test("serves a stored DMG from the Sites R2 binding", async () => {
  ...
  const response = await worker.fetch(
    new Request("http://localhost/download?version=0.6.3"),
    {
      DOWNLOADS: {
        async get(key) {
          assert.equal(key, "releases/v0.6.3/Pulse-0.6.3.dmg");
          return {
  ...
  assert.equal(
    ...
    'attachment; filename="Pulse-0.6.3.dmg"',
  );
  assert.equal(
    response.headers.get("content-type"),
    "application/x-apple-diskimage",
  );
  assert.equal(response.headers.get("x-pulse-version"), "0.6.3");
