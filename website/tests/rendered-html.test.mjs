...
test("redirects the stable download URL to a versioned request", async () => {
  ...
  assert.equal(
    ...
    "http://localhost/download?version=0.6.1",
  );
});
...
test("serves a stored DMG from the Sites R2 binding", async () => {
  ...
  const response = await worker.fetch(
    new Request("http://localhost/download?version=0.6.1"),
    {
      DOWNLOADS: {
        async get(key) {
          assert.equal(key, "releases/v0.6.1/Pulse-0.6.1.dmg");
          return {
  ...
  assert.equal(
    ...
    'attachment; filename="Pulse-0.6.1.dmg"',
  );
  assert.equal(
    response.headers.get("content-type"),
    "application/x-apple-diskimage",
  );
  assert.equal(response.headers.get("x-pulse-version"), "0.6.1");
